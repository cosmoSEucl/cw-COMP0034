...
from sklearn.preprocessing import PolynomialFeatures  # For best fit line
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H3("Regression Analysis: Factors Influencing Grant Amount", className="mb-3 text-primary"),
...
def update_regression_chart(years_range):
    filtered_df = df[
        (df['Award_Date'].dt.year >= years_range[0]) &
        (df['Award_Date'].dt.year <= years_range[1])
    ]
    ...
    X = filtered_df[['Duration_(Days)']]  # Using duration as the independent variable
    y = filtered_df['Amount_awarded']  # Amount awarded as the dependent variable

    # Handle NaN or inf values by replacing them with 0 or a more appropriate value
    X = np.nan_to_num(X)
    y = np.nan_to_num(y)
    ...
    # Add the scatter points for the actual data
    fig.add_trace(go.Scatter(
        x=X_test.flatten(),
        y=y_test,
        mode='markers',
    ...
    # Add the line for the regression model
    fig.add_trace(go.Scatter(
        x=X_test.flatten(),
        y=y_pred,
        mode='lines',
    ...
    # Update the layout
    fig.update_layout(
        title='Regression Analysis: Grant Amount vs. Duration',
